var calculadora = {
	...
	asignarEventosFormatoBotones: function(selector){
		...
		for (var i = 0; i<x.length;i++) {
			x[i].onmouseover = this.eventoPequenoBoton;
			x[i].onmouseleave = this.eventoGrandeBoton;
		};
	},

	eventoPequenoBoton: function(event){
		calculadora.PequenoBoton(event.target);
	},

	eventoGrandeBoton: function(event){
		calculadora.GrandeBoton(event.target);
	},

	PequenoBoton: function(elemento){
		var x = elemento.id;
